...
// Upstream PR #21257 — Qwen2 system regex:
// "(?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\\r\\n\\p{L}\\p{N}]?\\p{L}+|\\p{N}| ?[^\\s\\p{L}\\p{N}]+[\\r\\n]*|\\s*[\\r\\n]+|\\s+(?!\\S)|\\s+"
// Differs from llama3 only in the digit pattern (\p{N} vs \p{N}{1,3}). Without
// a custom handler, std::regex backtracking segfaults on long repeats.
static std::vector<size_t> unicode_regex_split_custom_qwen2(const std::string & text, const std::vector<size_t> & offsets) {
    ...
    for (auto offset : offsets) {
        ...
        for (size_t pos = offset_ini; pos < offset_end; /*pos++*/ ) {
            ...
            // regex: \p{N}    (single-digit, the only difference from llama3)
            if (flags.is_number) {
                pos++;
                _add_token(pos);
                continue;
...
static std::vector<size_t> unicode_regex_split_custom(const std::string & text, const std::string & regex_expr, const std::vector<size_t> & offsets) {
    ...
    if (regex_expr == "'s|'t|'re|'ve|'m|'ll|'d| ?\\p{L}+| ?\\p{N}+| ?[^\\s\\p{L}\\p{N}]+|\\s+(?!\\S)") {
        ...
        bpe_offsets = unicode_regex_split_custom_llama3(text, offsets);
            ...
            regex_expr == "(?:'[sS]|'[tT]|'[rR][eE]|'[vV][eE]|'[mM]|'[lL][lL]|'[dD])|[^\\r\\n\\p{L}\\p{N}]?\\p{L}+|\\p{N}| ?[^\\s\\p{L}\\p{N}]+[\\r\\n]*|\\s*[\\r\\n]+|\\s+(?!\\S)|\\s+") {
        // Upstream PR #21257 — Qwen2 regex dispatch (avoids std::regex stack overflow on long input).
