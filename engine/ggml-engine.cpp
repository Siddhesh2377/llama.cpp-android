...
ggml_engine_status ggml_engine_generate(
    ...
    if (n_prompt + sampling.n_predict > n_ctx) {
        // if prompt itself doesn't fit, error
        if (n_prompt > n_ctx) {
            return GGML_ENGINE_ERROR_OUT_OF_MEM;
        }
        // otherwise just limit generation
    ...
    llama_token eos = llama_vocab_eos(engine->vocab);
    llama_token eot = llama_vocab_eot(engine->vocab);
    ...
    while (n_generated < max_tokens) {
        ...
        for (int s = 0; s < sampling.stop_sequence_count && s < 8; s++) {
            if (sampling.stop_sequences[s] &&
                engine->response.size() >= strlen(sampling.stop_sequences[s]) &&
                engine->response.rfind(sampling.stop_sequences[s]) != std::string::npos) {
                // trim the stop sequence from response
                size_t pos = engine->response.rfind(sampling.stop_sequences[s]);
